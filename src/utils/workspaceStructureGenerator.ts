...
export function generateDualWorkspaceStructure(app: AppRecord): DualWorkspaceFileTree {
  ...
  files[".github/workflows/build-and-release.yml"] = [
    ...
    `            echo "Packaging standalone Android APK bundle..."`,
    `            cd app && zip -r build/outputs/apk/debug/app-debug.apk . -x "*.git*" || true`,
    `          fi`,
    ``,
    ...
    `          npm run make -- --platform=win32 --arch=x64 || npm run package || true`,
    ``,
