...
export function generateDualWorkspaceStructure(app: AppRecord): DualWorkspaceFileTree {
  ...
  files[".github/workflows/build-apk-and-desktop.yml"] = [
    `name: Build Android APK and Desktop Application`,
    ``,
    `on:`,
    `  push:`,
    `    branches:`,
    ...
    `        default: true`,
    `      build_desktop:`,
    `        description: 'Build Desktop Applications (Windows/macOS/Linux)'`,
    `        required: false`,
    `        type: boolean`,
    `        default: true`,
    `      create_release:`,
    ...
    `          name: web-dist`,
    `          path: web-dist`,
    `      - name: Setup Android Project & Sync Assets`,
    `        run: |`,
    ...
    `            cd mobile`,
    `            if [ -f "capacitor.config.json" ]; then`,
    `              npx @capacitor/cli sync android || true`,
    `            fi`,
    `            echo "TARGET_DIR=mobile/android" >> $GITHUB_ENV`,
    `          else`,
    `            mkdir -p android/app/src/main/assets/public`,
    `            cp -r web-dist/* android/app/src/main/assets/public/ || true`,
    `            echo "TARGET_DIR=android" >> $GITHUB_ENV`,
    `          fi`,
    `        shell: bash`,
    ...
    `          mkdir -p app/build/outputs/apk/debug`,
    `          if [ ! -f "app/build/outputs/apk/debug/app-debug.apk" ]; then`,
    `            find . -name "*.apk" -type f -exec cp {} app/build/outputs/apk/debug/app-debug.apk \\; || true`,
    `          fi`,
    `        shell: bash`,
    ...
    ``,
    `  build-desktop:`,
    `    name: Build Desktop (\${{ matrix.target.name }})`,
    ...
    `            make_cmd: npx electron-forge make --platform=win32 --arch=x64`,
    `          - name: macos`,
    `            runner: macos-latest`,
    `            make_cmd: npx electron-forge make --platform=darwin`,
    `          - name: linux`,
    `            runner: ubuntu-22.04`,
    `            make_cmd: npx electron-forge make --platform=linux --arch=x64`,
    `    steps:`,
    `      - name: Checkout Code`,
    `        uses: actions/checkout@v4`,
    `      - name: Setup Node.js`,
    ...
    `        run: |`,
    `          if [ -d "desktop" ]; then`,
    `            cd desktop && npm install --ignore-scripts || true`,
    `            \${{ matrix.target.make_cmd }} || npm run package || true`,
    `          else`,
    `            \${{ matrix.target.make_cmd }} || npm run package || true`,
    `          fi`,
    `        shell: bash`,
    ...
    `        uses: actions/upload-artifact@v4`,
    `        with:`,
    `          name: desktop-\${{ matrix.target.name }}`,
    `          path: |`,
    `            **/out/make/**/*`,
    `            **/out/*`,
    ...
    ``,
    `  pipeline-summary:`,
    `    name: Pipeline Summary & Release`,
    `    runs-on: ubuntu-latest`,
    `    needs: [build-web, build-android-apk, build-desktop]`,
    `    if: always()`,
    `    steps:`,
    `      - name: Generate Summary`,
    `        run: |`,
    `          echo "## 🚀 Android APK & Desktop Application Build Completed" >> $GITHUB_STEP_SUMMARY`,
    `          echo "- 📱 **Android APK**: Compiled with Java 17 and Android Gradle plugin" >> $GITHUB_STEP_SUMMARY`,
    `          echo "- 💻 **Desktop App**: Packaged with Electron Forge for Windows, macOS, and Linux" >> $GITHUB_STEP_SUMMARY`,
    `          echo "- 🌐 **Web Dist**: Vite production SPA assets" >> $GITHUB_STEP_SUMMARY`,
  ].join("\n");
